...
using Pathfinding;
using UnityEngine;
public class LBEnemy : EnemyBaseState
{
    ...
    public override void Defence()
    {
        ...
        aipath.destination = DefenceDirection;
    }
    ...
    public override void Pass(GameObject PlayerToPass)
    {
        ...
        if (PlayerToPass.GetComponent<Rigidbody>().velocity != Vector3.zero)
        {
            ...
            Debug.Log("transform + forward " + PlayerToPass.transform.position );
    ...
    public override void CheckAttackPos()
    {
        ...
        if (ball.GetPlayer() != null &&((ball.GetPlayer().transform.position.x < -10 || ball.GetPlayer().transform.position.x > 10 || Gamestart)))
        {
            Gamestart = true;
            if (GetBackYourPos())
            {
