...
using UnityEngine;
using Pathfinding;
public class AMCEnemy : EnemyBaseState
{
    ...
    public override void Pass(GameObject PlayerToPass)
    {
        ...
        if( ball.GetBallLocation() != null)
        {
            if (PlayerToPass.GetComponent<Rigidbody>().velocity != Vector3.zero )
            {
            
                force = ball.GetBallLocation().position - (PlayerToPass.transform.position + PlayerToPass.transform.forward);
            }
            else
            {   
        ...
        ball.GetComponent<Rigidbody>().AddForce(-force.normalized * PassSpeed * 10000 * Time.deltaTime);
    }
    ...
    public override void Shoot()
    {
       // Debug.Log("shoot");
        ...
        ball.GetComponent<Rigidbody>().velocity = Vector3.zero;
        ball.GetComponent<Rigidbody>().AddForce(goalDirection.normalized * ShootPower * 10000 * Time.deltaTime);
    }
